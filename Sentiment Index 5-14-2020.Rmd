...
Then the general and economic news is assessed using a custom library of 176 terms specifically focused on trade discourse whose sentiment were scored by the authors. 
...
EFsent <- rio::import("./Sentiment Analysis/EF_single_bing_year.csv")
BFsent <- rio::import("./Sentiment Analysis/BF_single_bing_year.csv")
...
The average sentiment is -4.469829
```{r}
...
The average sentiment is -2.234914
```{r}
mean(z$index)
```
...
### The Custom Sentiment Score is based on a custom dictionary of 176 terms specifically focused on trade discourse whose sentiment were scored by the authors
...
# The average sentiment for this new custom index is -9.075737
...
#ggplot(aes(x= year, y= index, color = index > -4.469829, size = 3)) +
  ...
  theme(legend.position = "none") +
   ...
   labs(title = "Custom Sentiment, China News", 
       ...
       caption = "Source: ProQuest - BroadFilter - Economic Filter Searches. Graphic by Rob Wells. 5-17-2020",
       x=" ",
       y="Sentiment: Negative -> Positive") 
