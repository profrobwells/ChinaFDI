---
title: "Company News Coverage Analysis 5-14-2020"
author: "Wells"
date: "5/14/2020"
output: html_document
---

# Analysis of Chinese Company News Coverage, 2000-2019
...
Updates Company Name Search 3-13-2020.R, July 18, 2019 - Based on Word String Search 6-14-19.R, Peter Navarro search
...
### Constructed table of names, extracted table
...
### Use Quanteda for phrase searching. Creates companies_toks table
```{r}
...
### Creating table summarizing the patterns
...
cos_pattern_sum <- companies_toks %>%
  ...
  count(pattern) %>% 
  arrange(desc(n))
head(cos_pattern_sum)
```

### Table with total mentions of top companies in five news organizations
...
companies_toks %>% 
  ...
  labs(title = "Total Articles, Top Chinese Cos. Investing in U.S.", 
       ...
       y="Count of Mentions",
       x="") 
#Created China Cos Articles3_17_2020
...
### In-depth coverage: Two or more mentions of company per article
...
glimpse(cos_sum3)
cos_sum3$total2 <- as.numeric(cos_sum3$total)
```

### Chart in-depth coverage

```{r}
cos_sum3 %>% 
  ggplot(aes(x = reorder(pattern, total2), y=total2, fill=pattern))+
  geom_col(show.legend = FALSE) +
  scale_y_continuous(limits=c(0, 1200)) +
  coord_flip()  +
  geom_text(aes(label = total), hjust=-.5, size = 3) +
  theme(axis.text.x = element_text(angle = 90, hjust = 1)) +
  labs(title = "In-Depth Articles, Top Chinese Cos. Investing in U.S.", 
       subtitle = "Articles with 2 or More Mentions of a Company",
       caption = "Cos. $2 bln U.S. investment. 353 Articles. Graphic by Rob Wells, 5-13-2020",
       y="Count of Articles: NYT, WSJ, LAT, WP, IUT",
       x="") 
#ggsave("./CompanyFilter/InDepth_Coverage_5-13_2020.png",device = "png",width=9,height=6)

#Created In-Depth Coverage Chinese Cos 5_13_2020
```


### In-depth coverage without CNOOC
...
cos_sum3 %>% 
  ...
  theme(axis.text.x = element_text(angle = 90, hjust = 1)) +
  labs(title = "In-Depth Articles, Top Chinese Cos. Investing in U.S.", 
       subtitle = "Articles with 2 or More Mentions of a Company. No CNOOC",
       caption = "Cos. $2 bln U.S. investment. 353 Articles. Graphic by Rob Wells, 5-13-2020",
       y="Count of Articles: NYT, WSJ, LAT, WP, IUT",
       x="") 
ggsave("./CompanyFilter/NoCNOOCInDepth_Coverage_5-13_2020.png",device = "png",width=9,height=6)

#Created NoCNOOCInDepth_Coverage_5-13_2020

```

### A closer look at the CNOOC coverage by year

```{r}
  glimpse(companies_toks)
```

### Build a table with CNOOC only coverage by year
...
### Now graph that CNOOC only table
...
companies_toks %>%
  ...
  geom_col(show.legend = FALSE) +
  theme(axis.text.x = element_text(angle = 90, hjust = 1)) +
  labs(title = "Media Mentions of CNOOC, 2000-2019", 
       subtitle = "Count of Mentions: NYT, WSJ, LAT, WP, IUT",
       caption = "353 Articles. Graphic by Rob Wells, 5-14-2020",
       y="Mentions of CNOOC",
       x="") 

ggsave("./CompanyFilter/CNOOC_year_5-14_2020.png",device = "png",width=9,height=6)

#Created CNOOC coverage by year, 5-14-2020
